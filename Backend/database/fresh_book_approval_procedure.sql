-- Fresh Book Approval Procedure
-- This procedure handles the complete book approval workflow
-- Including book creation, author processing, and request status updates
...
-- Main procedure for approving book requests
...
BEGIN
    ...
    SELECT 
        pr.PUBLISHER_ID,
        COALESCE(pbd.TITLE, 'Untitled') as title,
        COALESCE(pbd.ISBN, '') as isbn,
        pbd.PUBLISHED_DATE,
        COALESCE(pbd.PAGE_COUNT, 0) as page_count,
        COALESCE(pbd.LANGUAGE, 'English') as language,
        COALESCE(pbd.EDITION, '1st') as edition,
        COALESCE(pbd.PRICE, 0.00) as price,
        COALESCE(pbd.STOCK_QUANTITY, 0) as stock,
        COALESCE(pbd.DESCRIPTION, '') as description,
        COALESCE(pbd.COVER_URL, '/images/books/defaultbook.jpg') as cover_url,
        COALESCE(pbd.GENRE, 'General') as genre,
        pbd.AUTHORS,
        pbd.CATEGORY_ID
    INTO 
    ...
    -- Step 5: Process authors if they exist
    ...
    -- Step 7: Create notification for publisher
...
-- Test the procedure (commented out for production)
/*
-- Example usage:
CALL ApproveBookRequest(1, 1, 'Excellent book submission!', @msg, @book_id);
SELECT @msg as result_message, @book_id as new_book_id;
*/
